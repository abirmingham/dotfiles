...
"    endif
    let g:sqlplus_userid = input( "Database user: ", g:sqlplus_userid )
	call AE_getPasswd()
  endif
endfunction "}}}

function! AE_getPasswd() "{{{1
  let g:sqlplus_passwd = substitute( system( "db_password " . g:sqlplus_db . " " . g:sqlplus_userid ), "\n", "", "g" )
  if ( g:sqlplus_passwd == "" )
    let g:sqlplus_passwd = inputsecret( "Password for " . g:sqlplus_userid . ": " )
  endif
endfunction "}}}

function! AE_getDBsid( force ) "{{{1
  let g:sqlplus_db = input( "Database SID: ", g:sqlplus_db )
  call AE_getPasswd()
endfunction "}}}
...
function! AE_describeTable( tableName ) "{{{1
  let l:cmd = "set linesize 110\nprompt DESCRIBING TABLE '" . a:tableName . "'\ndesc " . a:tableName
  call AE_execQuery( l:cmd )
endfunction "}}}

function! AE_userSourcePrompt() "{{{1
  let l:procname = input( "Please enter the name of the procedure to source:  " )
  call AE_userSource( l:procname )
endfunction "}}}

function! AE_userSource( procName ) "{{{1
  let l:cmd = "set linesize 115\nselect text from user_source where name = upper('" . a:procName . "') order by line"
  call AE_execQuery( l:cmd )
endfunction "}}}
...
" Eventually I'd like to fix this but later.
"  let l:query = substitute( l:query, '\;$', '', '' )
  ...
  let l:question_i = 1
  let l:var = matchstr( l:query, '?' )
  while l:var > ""
    let l:var_val = input( "Enter value for bind param " . l:question_i . ": " )
    let l:query = substitute( l:query, '?', l:var_val, '' )
	let l:question_i = l:question_i + 1
    let l:var = matchstr( l:query, '?' )
  endwhile
...
" I changed these mappings a little because they don't work too well with perl and multi-user schemas
